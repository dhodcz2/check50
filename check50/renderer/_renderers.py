...
def to_html(slug, results, version, **kwargs):
    with open(TEMPLATES / "results.html") as f:
        content = f.read()

    # Combine additional keyword arguments into the render context
    render_context = dict(
        slug=slug,
        results=results,
        version=version,
...
def to_ansi(slug, results, version, _log=False, **kwargs):
    lines = [termcolor.colored(_("Results for {} generated by check50 v{}").format(slug, version), "white", attrs=["bold"])]

    # Include additional details from kwargs if provided
    if kwargs:
        lines.append(termcolor.colored(_("Additional Info:"), "blue", attrs=["bold"]))
        for key, value in kwargs.items():
            lines.append(termcolor.colored(f"    {key}: {value}", "blue"))
